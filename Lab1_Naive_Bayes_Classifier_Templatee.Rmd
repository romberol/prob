...
top10 <- head(words[order(words$Edgar.Alan.Poe, decreasing = TRUE),], 10)
...
df1 = melt(top10, "word")
df1%>%ggplot(aes(x=word, y=value)) +
  geom_bar(aes(fill=variable),stat="identity", position ="dodge") + 
  theme_bw()+ 
  theme(axis.text.x = element_text(angle=-40, hjust=.1))
#hist(edgar_top10$Edgar.Alan.Poe)#, xlim = c(0, 3), breaks = 9,
     #main = "Edgar Alan Poe's most used", xlab = "bla", col="red")
```
...
naiveBayes2 <- setRefClass("naiveBayes2",
       ...
       methods = list(
                    ...
                    fit = function(X, y)
                    {
                        ...
                        words[, 2:length(words)] <- words[, 2:length(words)]+1 # add ones
                        words <- data.frame(words[, -1], row.names = words$splitted)
                        words <- words %>% mutate_at(vars(1:3), funs(./sum(.)))
                        #View(words)
                        #for(i in 1:3){
                         # words[i] <- words[i]/sum(words[i])
                        #}
                    ...
                    predict = function(message)
                    {
                      ...
                      unnest_tokens(msg_df, splitted, splitted)%>% anti_join(stop_words, by=c("splitted" = "word"))
                      ...
                      for (i in results){
                        ...
                        inner_join(msg_df, rename_at(data1, 1, names(msg_df)[1]), by=names(msg_df)[1])
