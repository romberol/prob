...
message <- "hello my name is matthew how are you"
(grep("(\\w+\\s\\w+)", message, value = TRUE))
train$text<- gsub("[^'[:^punct:]]", "", tolower(train$text), perl = T)
tidy_text <- unnest_tokens(train, 'splitted', 'text', token="regex", pattern="(\\w+\\s\\w+)") %>%
=======
# note the power functional features of R bring us!
tidy_text <- unnest_tokens(train, 'splitted', 'text', token="words") %>%
>>>>>>> 7005050ca5b3a3ca42802f29f151eaf0ca50339f
